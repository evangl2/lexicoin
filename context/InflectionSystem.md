...
```typescript
import { generateInflectionTable } from 'schemas/inflection/InflectionEngine';
...
| 文件 | 用途 |
|------|------|
| `schemas/SenseEntity.schema.ts` | 主 Schema（含 LinguisticTrait / TraitId / WordFamily 类型定义） |
| `schemas/KeyFormsDictionary.config.ts` | 不规则变形位置字典（VERB / NOUN / ADJ） |
| `schemas/inflection/InflectionTypes.ts` | 形变引擎输出类型 |
| `schemas/inflection/InflectionEngine.ts` | 主调度器 + 语言模块注册 |
| `schemas/inflection/rules/en.ts` | 英语规则模块（完整实现） |
| `schemas/UserSenseProgress.schema.ts` | 学习者进度（本地 IndexedDB） |
