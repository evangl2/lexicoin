...
import { sensesToCards, senseToCard } from "@core/pipelines/senseToCard";
...
export const useCardManager = () => {
    ...
    const storeCard = useCallback((uid: string) => {
        setItems(prev => {
            ...
            // and skips re-render if the item is not found.
    ...
    useEffect(() => {
        ...
        const handleAssetLoaded = (msg: any) => {
            ...
            setItems(prev => {
                const idx = prev.findIndex(item => item.cardData.rawSense.uid === entry.uid);
                if (idx === -1) {
        ...
        const handleAssetError = (msg: any) => {
            ...
            setItems(prev => {
                // Optimization: Use findIndex to update a single item. Avoids O(N) array allocation
                // and skips re-render if the item is not found.
                const idx = prev.findIndex(item => item.cardData.rawSense.uid === assetId);
                if (idx === -1) return prev;
                const next = [...prev];
                next[idx] = {
                    ...next[idx],
